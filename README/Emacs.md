...
- `U`: redo
- `C-a`: 数値を増やす
- normal state の `C-x`: 数値を減らす
- `C-x` prefix (`C-x C-f`、`C-x d` など) は insert state で使う
- `>` / `<`: インデント
...
- `SPC d`: 現在のバッファのディレクトリを Dired で開く
- `C-x d`: ディレクトリを指定して Dired で開く
- `C-x C-f`: ファイルまたはディレクトリを開く
- `j` / `k`: 移動
- `RET` または `l`: ファイルを開く
- `h` または `^`: 親ディレクトリへ戻る
- `q`: filer を閉じる
