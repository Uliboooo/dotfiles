...
pinentry は端末内で表示する。Fish は Home Manager の `programs.fish` では管理して
いないため、シェル統合の一部を `.config/fish/config.fish` に手書きする。

## 2026-08-27

### pinentry-curses を gpg-agent に接続

触ったファイル: `home/common_user.nix`, `.config/fish/config.fish`

`services.gpg-agent.pinentry.package` に `pkgs.pinentry-curses` を指定する。パッケージを
